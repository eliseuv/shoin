...
mod tests {
    ...
    /// Feed Alt-modified character keys, one per char.
    fn feed_alt(app: &mut App, keys: &str) {
        for c in keys.chars() {
    ...
    #[test]
    fn alt_h_l_indent_and_outdent_a_bullet_item() {
        let mut app = app_with("- one\n- two\n");
        feed_alt(&mut app, "l");
        assert_eq!(text(&app), "    - one\n- two\n");
        feed_alt(&mut app, "h");
    ...
    #[test]
    fn alt_l_carries_nested_children_along() {
    ...
    #[test]
    fn alt_k_j_swap_sibling_bullet_items() {
        let mut app = app_with("- one\n- two\n- three\n");
    ...
    #[test]
    fn alt_j_renumbers_ordered_siblings_after_a_move() {
    ...
    #[test]
    fn alt_j_moves_an_item_with_a_wrapped_continuation_line() {
        let mut app = app_with("- one\n  wraps here\n- two\n");
        app.buffer.cursor = Cursor::new(0, 0);
        feed_alt(&mut app, "j");
        assert_eq!(text(&app), "- two\n- one\n  wraps here\n");
    ...
    #[test]
    fn alt_k_j_h_are_a_noop_at_the_edges_of_a_list() {
        let mut app = app_with("- one\n- two\n");
        app.buffer.cursor = Cursor::new(0, 0);
        feed_alt(&mut app, "k"); // no previous sibling
        assert_eq!(text(&app), "- one\n- two\n");
        app.buffer.cursor = Cursor::new(1, 0);
        feed_alt(&mut app, "j"); // no next sibling
        assert_eq!(text(&app), "- one\n- two\n");
        feed_alt(&mut app, "h"); // already at depth 0 — shift_lines itself no-ops
        assert_eq!(text(&app), "- one\n- two\n");
    }
